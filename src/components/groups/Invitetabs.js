                ...
                <div className="tab-content ">
                    <div className="tab-pane fade active show" id="InviteMyFriends" role="tabpanel" aria-labelledby="InviteMyFriends-tab">
                        ...
                        {MyFriends.length === 0 ? <p className='text-center my-3'>No Friends Found!</p> : <>
                            {MyFriends.map((card, i) => (
                                <div className="card rounded-5 mt-3" key={i} id={UserProfiledata?.data?.id === card?.friend?.id ? card?.user?.id : card?.friend?.id}>
                                    <div className="card-body align-items-center d-flex justify-content-between py-2">
                                        <div className="d-flex align-items-center ">
                                            {UserProfiledata?.data?.id === card?.friend?.id ? <>
                                                {card.user?.profile_photo === null ?
                                                    <Image src={`/assets/images/Modal/Avatar.png`} alt="" width={100} height={100} className='post-profile'></Image>
                                                    : <Image loader={imgurl} src={card.user?.profile_photo?.url} alt="" width={100} height={100} className='post-profile object-fit-cover'></Image>
                                                }
                                            </> : <> {
                                            ...
                                            <p className='papa mb-0 clr-text fw-bold ms-2'>{UserProfiledata?.data?.id === card?.friend?.id ? card?.user?.name : card?.friend?.name}</p>
                                        </div>
