...
const MyGroups = ({ postdone, endpoint }) => {
    const searchParamsgroup = useSearchParams()
    const tabgroups = searchParamsgroup.get('activity_tab')
    const [activeTabgroup, setactiveTabgroup] = useState('')
    useEffect(() => {
        if (tabgroups) {
            setactiveTabgroup(tabgroups)
        }
        console.log('tab', activeTabgroup)
    }, [tabgroups, searchParamsgroup])
    ...
    useEffect(() => {
        ...
        if (activeTabgroup === 'my_group') {
            if (CurrentPagefrnd === 1 && Datafrnds.length === 0) {
                fetchGroupPosts(CurrentPagefrnd);
            }
        }
    }, [CurrentPagefrnd, token, postdone, isdlt, EditDone, endpoint,activeTabgroup]);
    ...
    return (
        <>
                    ...
                    return (
                        <div className='post-card mt-4 ' key={i}>
                            <div className='post-card-body ms-md-3 mb-3 back-border rounded-3 col-xxl-5 col-lg-7 col-md-8' >
                                    ...
                                    {item?.created_by?.id === UserProfiledata?.data?.id &&
                                        <li className='ms-auto'>
                                            ...
                                            <ul className="dropdown-menu">
                                                <li><Link className="dropdown-item " href={`#`} onClick={() => setgrpid(item.id)} data-bs-toggle="modal" data-bs-target="#DltPostgroups" >Delete Post</Link></li>
                                                <li><Link className="dropdown-item " href={`#`} data-bs-toggle="modal" onClick={() => setprevData(item)} data-bs-target="#groupseditpost" >Edit Post</Link></li>
                                ...
                                <FancyBoxPost images={item?.media[0]?.media?.url} fancyBoxId={`groupsFancyBox${i}`} modalOpen={PostmodalOpen} closeModal={PostcloseModal} selectedImage={PostselectedImage} setSelectedImage={setPostSelectedImage} para={item.post_text} name={item.created_by.name} profile={item?.created_by?.profile_photo} time={timeDiffString} item={item} dislikepost={dislikepost} handleToggle={handleToggle} likepost={likepost} likecount={likecount} Comments={Comments} getcomment={getcomment} cmntloader={cmntloader} />
                                    ...
                                    <div className="d-flex ">
                                        ...
                                        <div className="col-6 text-center py-2">
                                            <span className='pointer' onClick={() => { PostopenModal(0), getcomment(item.id), setcmntloader(true) }}
                                                data-bs-toggle="modal" data-bs-target={`#groupsFancyBox${i}`}>
            ...
            <DeletePost grpid={grpid} setisdlt={setisdlt} isdlt={isdlt} modalid={'DltPostgroups'} />
            <EditPostModal postin={'profile'} prevData={prevData} setEditDone={setEditDone} EditDone={EditDone} editmodalid={'groupseditpost'} />
        </>
    )
}
