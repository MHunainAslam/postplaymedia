...
const AllGroups = ({ postdone, endpoint }) => {
    ...
    const [AllGrpPosts, setAllGrpPosts] = useState([])
    const [likecount, setlikecount] = useState('')
    const [loading, setLoading] = useState(1)
    const [AllGrpCurrentPagefrnd, setAllGrpCurrentPagefrnd] = useState(1)
    const [AllGrpTotalPagesfrnd, setAllGrpTotalPagesfrnd] = useState()
    const [Datafrnds, setDatafrnds] = useState([])
    const [AllGrptotalMemberfrnd, setAllGrptotalMemberfrnd] = useState(1)
    const router = useRouter()
    const [AllFrndsData, setAllFrndsData] = useState([])
    const [AllGrpUserDataLoader, setAllGrpUserDataLoader] = useState(true)
    ...
    const fetchAllGrpPosts = async (page) => {
        try {
            ...
            if (data.success) {
                ...
                setAllGrpTotalPagesfrnd(data.data.last_page);
                setAllGrptotalMemberfrnd(data.data.total);
                setAllGrpUserDataLoader(false)
                setAllGrpUserDataLoader(false)
            } else {
    ...
    const fetchAllGrpPostss = async (page) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setAllGrpPosts((prevMessages) => [...prevMessages, ...data?.data?.data]);
                setAllGrpCurrentPagefrnd(data.data.current_page);
                setAllGrpTotalPagesfrnd(data.data.last_page);
    ...
    useEffect(() => {
        ...
        if (AllGrpCurrentPagefrnd === 1 && Datafrnds.length === 0) {
            fetchAllGrpPosts(AllGrpCurrentPagefrnd);
        }
    }, [AllGrpCurrentPagefrnd, token]);
    const handleLoadMorefrnd = () => {
        if (AllGrpCurrentPagefrnd < AllGrpTotalPagesfrnd && !loading) {
            setLoading(true);
            fetchAllGrpPostss(AllGrpCurrentPagefrnd + 1);
        }
    };
    ...
    const handleToggle = (postId) => {
        setAllGrpPosts(prevData => prevData.map(post => {
            if (post.id === postId) {
