...
const Allmembersadmin = ({ postdone, endpoint }) => {
    ...
    const [AllMemberPosts, setAllMemberPosts] = useState([])
    const [likecount, setlikecount] = useState('')
    const [loading, setLoading] = useState(1)
    const [AllMemberCurrentPagefrnd, setAllMemberCurrentPagefrnd] = useState(1)
    const [AllMemberTotalPagesfrnd, setAllMemberTotalPagesfrnd] = useState()
    const [Datafrnds, setDatafrnds] = useState([])
    const [AllMembertotalMemberfrnd, setAllMembertotalMemberfrnd] = useState(1)
    const router = useRouter()
    const [AllFrndsData, setAllFrndsData] = useState([])
    const [AllMemberUserDataLoader, setAllMemberUserDataLoader] = useState(true)
    ...
    const fetchAllMemberPosts = async (page) => {
        try {
            ...
            if (data.success) {
                ...
                setAllMemberPosts(data.data.data);
                setAllMemberCurrentPagefrnd(data.data.current_page);
                setAllMemberTotalPagesfrnd(data.data.last_page);
                setAllMembertotalMemberfrnd(data.data.total);
                setAllMemberUserDataLoader(false)
                setAllMemberUserDataLoader(false)
            } else {
    ...
    const fetchAllMemberPostss = async (page) => {
        ...
        try {
            ...
            if (data.success) {
                // Prepend new messages to the beginning of the array
                setAllMemberPosts((prevMessages) => [...prevMessages, ...data?.data?.data]);
                setAllMemberCurrentPagefrnd(data.data.current_page);
                setAllMemberTotalPagesfrnd(data.data.last_page);
    ...
    useEffect(() => {
        ...
        if (AllMemberCurrentPagefrnd === 1 && Datafrnds.length === 0) {
            fetchAllMemberPosts(AllMemberCurrentPagefrnd);
        }
    }, [AllMemberCurrentPagefrnd, token]);
    const handleLoadMorefrnd = () => {
        if (AllMemberCurrentPagefrnd < AllMemberTotalPagesfrnd && !loading) {
            setLoading(true);
            fetchAllMemberPostss(AllMemberCurrentPagefrnd + 1);
        }
    };
    ...
    const handleToggle = (postId) => {
        setAllMemberPosts(prevData => prevData.map(post => {
            if (post.id === postId) {
    ...
    return (
        <>
            {AllMemberUserDataLoader ? <div className="w-100 text-center mt-4">
                <span className="spinner-grow spinner-grow-sm mx-2 clr-primary" aria-hidden="true"></span>
                <span className="spinner-grow spinner-grow-sm mx-2 clr-primary" aria-hidden="true"></span>
                <span className="spinner-grow spinner-grow-sm mx-2 clr-primary" aria-hidden="true"></span>
            </div> :
                AllMemberPosts.length == 0 &&
                    ...
                    return (
                        ...
                        <div className='post-card mt-4 ' key={i}>
                            <div className='post-card-body ms-md-3 mb-3 back-border rounded-3 col-xxl-5 col-lg-7 col-md-8' >
                                    ...
                                    {item?.created_by?.id === UserProfiledata?.data?.id &&
                                        <li className='ms-auto'>
                                            ...
                                            <ul className="dropdown-menu">
                                                <li><Link className="dropdown-item " href={`#`} onClick={() => setgrpid(item.id)} data-bs-toggle="modal" data-bs-target="#DltPostmentions" >Delete Post</Link></li>
                                                <li><Link className="dropdown-item " href={`#`} data-bs-toggle="modal" onClick={() => setprevData(item)} data-bs-target="#mentionseditpost" >Edit Post</Link></li>
                                ...
                                <FancyBoxPost images={item?.media[0]?.media?.url} fancyBoxId={`mentionsFancyBox${i}`} modalOpen={PostmodalOpen} closeModal={PostcloseModal} selectedImage={PostselectedImage} setSelectedImage={setPostSelectedImage} para={item.post_text} name={item.created_by.name} profile={item?.created_by?.profile_photo} time={timeDiffString} item={item} dislikepost={dislikepost} handleToggle={handleToggle} likepost={likepost} likecount={likecount} Comments={Comments} getcomment={getcomment} cmntloader={cmntloader} />
                                    ...
                                    <div className="d-flex ">
                                        ...
                                        <div className="col-6 text-center py-2">
                                            <span className='pointer' onClick={() => { PostopenModal(0), getcomment(item.id), setcmntloader(true) }}
                                                data-bs-toggle="modal" data-bs-target={`#mentionsFancyBox${i}`}>
            ...
            <DeletePost grpid={grpid} setisdlt={setisdlt} isdlt={isdlt} modalid={'DltPostmentions'} />
            <EditPostModal postin={'profile'} prevData={prevData} setEditDone={setEditDone} EditDone={EditDone} editmodalid={'mentionseditpost'} />
        </>
    )
}
