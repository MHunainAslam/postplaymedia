...
const AllAthelete = ({setatheletecount}) => {
    ...
    const fetchcoaches = async (page) => {
        try {
            const response = await fetch(
                `${APP_URL}/api/users?role_id=3&per_page=20&page=${page}&search=${coachSearch}`,
                {
                    headers: {
            ...
            if (data.success) {
                ...
                setatheletecount(data?.data?.total)
    ...
    const fetchcoachess = async (page) => {
        try {
            const response = await fetch(
                `${APP_URL}/api/users?role_id=3&per_page=20&page=${page}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        // Add other headers if needed
